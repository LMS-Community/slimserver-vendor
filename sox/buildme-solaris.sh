...
export PATH=/opt/gcc-$GCCPATH/bin:/usr/gnu/bin:$PATH
CC=`which gcc`; export CC
CXX=$GCCPATH/bin/c++; export CXX
CPP=$GCCPATH/bin/cpp; export CPP
...
patch -p0 < ../06-filio-SunOS.patch >> $LOG
echo "Configuring..."
...
patch -p0 < ../05-configure-SunOS.patch
echo "Configuring..."
CC=$CC CXX=$CXX \
...
./configure CFLAGS="$CPF" LDFLAGS="$LDF" OPUS_CFLAGS="-I$PWD/../opusfile-$OPUSFILE/include" OPUS_LIBS="-L$PWD/../opusfile-$OPUSFILE/.libs -lopusfile -lopus" --without-ao --without-pulseaudio --disable-openmp --with-flac --with-oggvorbis --with-opus --with-mp3 --with-wavpack --without-id3tag --without-lame --without-png --without-ladspa --disable-shared --without-oss --without-alsa --disable-symlinks --without-coreaudio --without-sndfile --prefix $OUTPUT >> $LOG
echo "Running make"
gmake -j $CORES >> $LOG
